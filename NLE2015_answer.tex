...
We did try to augment the MFCC with the warping factors obtained in the standard way (in this case there is only one scalar factor per utterance) the results of these experiments are reported in Table 5 row {\em Warp + MFCC}.
...
Yet, we totally agree with you on the fact that in a more advance stage the system should include some kind of sequence training and an analysis of its effects on different age and gender groups. But once again this is not in the scope of this paper.
